...
- **Aviso de "Pasta já existe":** Ocorre se o candidato rodar o comando novamente usando um número PP que já foi preparado antes. O sistema perguntará se deseja sobrescrever. **Atenção:** se o candidato confirmar (digitando `s`), **todo o trabalho que ele já desenvolveu na pasta será apagado e substituído pelos arquivos originais vazios**. Instrua-o a cancelar digitando `N` se não quiser perder o que já fez.
